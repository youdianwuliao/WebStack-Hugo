...
输出：xinshi/scans/v{1,2}-NNN.webp        1600px 宽，q84，阅读用（约 230KB/叶）
      xinshi/scans/thumb/v{1,2}-NNN.webp   320px 宽，q75，目录缩略图（约 8KB/叶）
...
WORKERS = 6
MIN_BYTES = 3000          # 缩略图约 8KB，阈值取 3KB 以免误判失败
...
def main():
    ...
    def work(t):
        src, big, thumb = t
        return convert(src, big, 1600, 84) and convert(src, thumb, 320, 75)
